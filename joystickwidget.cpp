...
float JoystickWidget::calcMoveSpeedStepAt(int index)
{
    ...
    float multiplier = ((m_moveAdjustmentWeight * (std::pow(xVal, m_moveAdjustment))) + ((1 - m_moveAdjustmentWeight) * xVal));
    return (m_maxDistFromCenter - ((multiplier * m_maxMoveSpeed) * (m_maxDistFromCenter / m_maxMoveSpeed)));
}

float JoystickWidget::calcZoomSpeedStepAt(int index)
{
    float xVal = (float) index / m_maxZoomSpeed;
    float multiplier = ((m_zoomAdjustmentWeight * (std::pow(xVal, m_zoomAdjustment))) + ((1 - m_zoomAdjustmentWeight) * xVal));
    return ((m_maxZoomSpeed * m_degPerZoom) - ((multiplier * m_maxZoomSpeed) * m_degPerZoom));
}
...
void JoystickWidget::updateMoveSpeedSteps()
{
    ...
    for (int i = 0; i < m_maxMoveSpeed; i++)
    {
        ...
        QRectF bounds(-(step.radius), -(step.radius), (step.radius * 2), (step.radius * 2));
...
void JoystickWidget::updateZoomSpeedSteps()
{
    ...
    for (int i = 0; i < m_maxZoomSpeed; i++)
    {
        ...
        step.dispDeg = (m_zoomArcSpan * step.deg) / (m_maxZoomSpeed * m_degPerZoom);
...
void JoystickWidget::updateState()
{
    ...
    if (m_zoomSpeedSteps.last().deg < std::abs(m_scrollWheelPos))
    {
        ...
        {
            if (m_zoomSpeedSteps.at(i).deg < std::abs(m_scrollWheelPos))
                m_state.zoom = m_zoomSpeedSteps.at(i).speed * m_zoomDirection;
        }
    } else {
        m_state.zoom = 0;
    }
...
void JoystickWidget::emitChanges()
{
    ...
    if (m_state.direction != m_prevState.direction || m_state.speed != m_prevState.speed)
    {
...
void JoystickWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        m_shouldBindToMousePos = QLineF(centerPoint(), event->pos()).length() <= m_maxDistFromCenter;
...
void JoystickWidget::wheelEvent(QWheelEvent *event)
{
    ...
    if (std::abs(m_scrollWheelPos) > m_zoomSpeedSteps.first().deg)
        m_scrollWheelPos = m_zoomSpeedSteps.first().deg * m_zoomDirection;
    emitChanges();
}
...
void JoystickWidget::paintEvent(QPaintEvent * /*event*/)
{
    ...
    if (m_scrollWheelPos)
    {
        painter.setPen(QPen(m_scrollWheelPos > 0 ? Qt::green : Qt::red, 2));
        painter.drawArc(m_moveSpeedSteps.first().bounds.adjusted(-2, -2, 2, 2), 90 * 16,
                        (m_zoomArcSpan * std::abs(m_scrollWheelPos)) / m_zoomSpeedSteps.first().deg * m_zoomDirection * -16);
    }
    ...
    float angles[8] = {337.5, 292.5, 247.5, 202.5, 157.5, 112.5, 67.5, 22.5};
    for (int i = 0; i < 8; i++)
    {
