...
SettingsDialog::SettingsDialog(QWidget *parent, Qt::WindowFlags f) : QWidget(parent, f)
{
    setWindowTitle(LOCALE("PTZControllerSettings"));
    ...
    m_ipRegex = QRegExp("^" + ipRange + "\\." + ipRange + "\\." + ipRange + "\\." + ipRange + "$");
    QRegExpValidator *ipValidator = new QRegExpValidator(m_ipRegex, connectionGroup);
    m_ipField = new QLineEdit(connectionGroup);
    m_ipField->setValidator(ipValidator);
    connect(m_ipField, SIGNAL(textChanged(QString)), this, SLOT(onIpChanged(QString)));

    QLabel *portLbl = new QLabel(LOCALE("Port") + ": ", connectionGroup);
    m_portRegex = QRegExp("^(0|[1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])$");
    QRegExpValidator *portValidator = new QRegExpValidator(m_portRegex, connectionGroup);
    m_portField = new QLineEdit(connectionGroup);
    m_portField->setValidator(portValidator);
    connect(m_portField, SIGNAL(textChanged(QString)), this, SLOT(onPortChanged(QString)));
    ...
    QLabel *moveAdjLbl = new QLabel(LOCALE("MovementAdjustment") + ": ", joystickGroup);
    m_moveAdjSlider = new QSlider(Qt::Horizontal, joystickGroup);
    m_moveAdjSlider->setRange(100, 400);
    connect(m_moveAdjSlider, SIGNAL(valueChanged(int)), this, SLOT(onMoveAdjChanged(int)));

    QLabel *moveAdjWeightLbl = new QLabel(LOCALE("MovementAdjustmentWeight") + ": ", joystickGroup);
    m_moveAdjWeightSlider = new QSlider(Qt::Horizontal, joystickGroup);
    m_moveAdjWeightSlider->setRange(0, 100);
    connect(m_moveAdjWeightSlider, SIGNAL(valueChanged(int)), this, SLOT(onMoveAdjWeightChanged(int)));

    QLabel *zoomAdjLbl = new QLabel(LOCALE("ZoomAdjustment") + ": ", joystickGroup);
    m_zoomAdjSlider = new QSlider(Qt::Horizontal, joystickGroup);
    m_zoomAdjSlider->setRange(100, 400);
    connect(m_zoomAdjSlider, SIGNAL(valueChanged(int)), this, SLOT(onZoomAdjChanged(int)));

    QLabel *zoomAdjWeightLbl = new QLabel(LOCALE("ZoomAdjustmentWeight") + ": ", joystickGroup);
    m_zoomAdjWeightSlider = new QSlider(Qt::Horizontal, joystickGroup);
    m_zoomAdjWeightSlider->setRange(0, 100);
    connect(m_zoomAdjWeightSlider, SIGNAL(valueChanged(int)), this, SLOT(onZoomAdjWeightChanged(int)));

    QLabel *degPerZoomLbl = new QLabel(LOCALE("ZoomSensitivity") + ": ", joystickGroup);
    ...
    connect(m_degPerZoomSlider, SIGNAL(valueChanged(int)), this, SLOT(onDegPerZoomChanged(int)));

    QLabel *zoomArcSpanLbl = new QLabel(LOCALE("ZoomIndicatorRadius") + ": ", joystickGroup);
    ...
    connect(m_zoomArcSpanSlider, SIGNAL(valueChanged(int)), this, SLOT(onZoomArcSpanChanged(int)));
    ...
    connect(m_autoPowerOn, SIGNAL(stateChanged(int)), this, SLOT(onAutoPowerChanged()));

    m_autoPowerOff = new QCheckBox(LOCALE("AutoPowerOff"), powerGroup);
    connect(m_autoPowerOff, SIGNAL(stateChanged(int)), this, SLOT(onAutoPowerChanged()));
    ...
    QLabel *presetRowsLbl = new QLabel(LOCALE("PresetRows") + ": ", presetGroup);
    m_presetRowsBox = new QSpinBox(presetGroup);
    m_presetRowsBox->setRange(2, 6);
    m_presetRowsBox->setValue(4);
    connect(m_presetRowsBox, SIGNAL(valueChanged(int)), this, SLOT(onPresetRowsChanged(int)));

    QLabel *presetColumnsLbl = new QLabel(LOCALE("PresetColumns") + ": ", presetGroup);
    m_presetColumnsBox = new QSpinBox(presetGroup);
    m_presetColumnsBox->setRange(1, 5);
    m_presetColumnsBox->setValue(3);
    connect(m_presetColumnsBox, SIGNAL(valueChanged(int)), this, SLOT(onPresetColumnsChanged(int)));
    ...
    QDialogButtonBox *closeBtns = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel);

    connect(closeBtns, &QDialogButtonBox::accepted, this, &SettingsDialog::onOkClicked);
    connect(closeBtns, &QDialogButtonBox::rejected, this, &SettingsDialog::onCancelClicked);
...
void SettingsDialog::loadSettings()
{
	...
	if (data) {
        ...
        m_moveAdjWeightSlider->setValue(obs_data_get_int(data, "moveAdjWeight"));
        m_zoomAdjSlider->setValue(obs_data_get_int(data, "zoomAdj"));
        m_zoomAdjWeightSlider->setValue(obs_data_get_int(data, "zoomAdjWeight"));
        m_degPerZoomSlider->setValue(obs_data_get_int(data, "degPerZoom"));
        m_zoomArcSpanSlider->setValue(obs_data_get_int(data, "zoomArcSpan"));

		m_autoPowerOn->setChecked(obs_data_get_bool(data, "autoPowerOn"));
		m_autoPowerOff->setChecked(obs_data_get_bool(data, "autoPowerOff"));

        m_presetRowsBox->setValue(obs_data_get_int(data, "presetRows"));
        m_presetColumnsBox->setValue(obs_data_get_int(data, "presetColumns"));
...
void SettingsDialog::saveSettings()
{
	...
	if (file) {
        ...
        obs_data_set_string(data, "ip", m_ipField->text().toUtf8().constData());
        obs_data_set_string(data, "port", m_portField->text().toUtf8().constData());

        obs_data_set_int(data, "moveAdj", m_moveAdjSlider->value());
        obs_data_set_int(data, "moveAdjWeight", m_moveAdjWeightSlider->value());
        obs_data_set_int(data, "zoomAdj", m_zoomAdjSlider->value());
        obs_data_set_int(data, "zoomAdjWeight", m_zoomAdjWeightSlider->value());
        obs_data_set_int(data, "degPerZoom", m_degPerZoomSlider->value());
        obs_data_set_int(data, "zoomArcSpan", m_zoomArcSpanSlider->value());

	obs_data_set_bool(data, "autoPowerOn", m_autoPowerOn->isChecked());
	obs_data_set_bool(data, "autoPowerOff", m_autoPowerOff->isChecked());

        obs_data_set_int(data, "presetRows", m_presetRowsBox->value());
        obs_data_set_int(data, "presetColumns", m_presetColumnsBox->value());
...
void SettingsDialog::closeEvent(QCloseEvent *event)
{
    if (m_needsToSave)
    {
        ...
        msgBox = QMessageBox::question(this, LOCALE("SaveSettings"), LOCALE("SaveSettingsMsg"),
